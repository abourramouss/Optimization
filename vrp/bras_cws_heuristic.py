...
def biased_randomization(savingsList, beta):
    biased_savingsList = []
    while savingsList:
        index = int(math.log(random.random()) / math.log(1 - beta))
        index = index % len(savingsList)
        biased_savingsList.append(savingsList.pop(index))
    return biased_savingsList


def create_solution_from_savings(savingsList, nodes):
    ...
    while len(savingsList) > 0:  # List is not empty
        ...
        if isMergeFeasible == True:
            iEdge = getDepotEdge(iRoute, iNode)
...
# Multi-start randomized CWS heuristic
def multi_start_cws(
    num_starts,
    beta,
    nodes,
    ...
    for _ in range(num_starts):
        # Create a biased-randomized savings list, we could also use the best solution list
        randomized_savingsList = biased_randomization(savingsList.copy(), beta)

        # Create a solution from the biased-randomized savings list
        current_solution = create_solution_from_savings(randomized_savingsList, nodes)

        # Evaluate the cost of the current solution
        current_cost = current_solution.cost

        # If the current solution is better, update the best solution and cost
...
# Parameters for the multi-start process
num_starts = 100  # Number of multi-start iterations
beta = 0.20  # Bias parameter for the geometric probability distribution

# Run the multi-start randomized CWS heuristic
best_solution, best_cost = multi_start_cws(num_starts, beta, nodes)

# Output the best solution
print("Best Cost of CW savings sol =", "{:.2f}".format(best_cost))
for route in best_solution.routes:
    s = str(0)
    for edge in route.edges:
        s += " - " + str(edge.end.ID)
    print("Best Route: " + s + " || cost = " + "{:.2f}".format(route.cost))
...
G = nx.Graph()
fnode = best_solution.routes[0].edges[0].origin
...
for j, route in enumerate(best_solution.routes):
